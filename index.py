# before running this install the dependencies by pip install -r requirements.txt
...
# mutualfundbot is the name of the table which is having 4 fied id, query, addressed, query_type
# mutualfund_cb is the name of the database
def insert_to_db(qt, q, a):
    sql = "insert into `mutualfundbot` (`query`, `addressed`, `query_type`) values ('{0}', '{1}', '{2}')".format(
        q, a, qt)
    print(sql)
    con = create_engine("mysql+pymysql://root:@127.0.0.1:3307/mutualfund_cb")
    con.execute(sql)

# vaderSentiment analaysis to give the score depending upon the query type
...
def lookup(terms, query):
    enhanced = [l.name() for term in terms for syn in wordnet.synsets(term)
                for l in syn.lemmas()]
...
def parser():
    ...
    response = make_response(json.dumps(
        {"fulfillmentText": outcome, "source": "MHSSCOE"}))
